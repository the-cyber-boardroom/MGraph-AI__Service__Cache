/* ═══════════════════════════════════════════════════════════════════════════════
   Cache Service Browser - Base Component Class
...
class BaseComponent extends HTMLElement {
    ...
    /**
     * Override in subclass for cleanup on disconnect
     */
    onDisconnected() {
        // Subclass hook
    }
    ...
    async loadResources() {
        ...
        const [sharedCss, componentCss, templateHtml] = await Promise.all([
            this.fetchCss(ComponentPaths.sharedCss),
    ...
    /**
     * Add event listener with tracking for cleanup (on shadow root elements)
    ...
    addTrackedListener(target, eventType, handler, options = {}) {
        if (!target) {
            console.warn(`[${this.tagName}] Cannot add listener to null target`);
            return;
        }
    ...
    addDocumentListener(eventType, handler, options = {}) {
        ...
        this._documentListeners.push({ eventType, handler: boundHandler, options });
    }
    ...
    cleanup() {
        ...
        for (const { eventType, handler, options } of this._documentListeners) {
    ...
    setState(newState) {
        ...
        if (this._isReady && typeof this.render === 'function') {
            this.render();
        }
    }
}
