...
git remote add template https://github.com/the-cyber-boardroom/MGraph-AI__Service__Cache.git || true
git fetch template
...
find . -type f \( -name "*.py" -o -name "*.md" -o -name "*.yml" -o -name "*.yaml" -o -name "*.toml" -o -name "*.sh" -o -name "*.txt" \) | while read file; do
    ...
    sed -i.tmp "s/mgraph_ai_service_cache/$SERVICE_NAME/g" "$file"
    sed -i.tmp "s/MGraph-AI__Service__Cache/$REPO_NAME/g" "$file"
    sed -i.tmp "s/MGraph-AI Service Cache/$SERVICE_DISPLAY_NAME/g" "$file"
