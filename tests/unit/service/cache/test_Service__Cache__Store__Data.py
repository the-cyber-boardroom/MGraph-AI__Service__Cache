...
class test_Cache__Service__Store__Data(TestCase):
    ...
    def test_store_data__string(self):                                                          # Test string data storage
        with self.service__store_data as _:
            data_key     = Safe_Str__File__Path('an/data/folder')
            data_file_id = Safe_Str__Id('an-file')
            ...
            expected_data_file = f'{self.test_namespace}/data/semantic-file/logs/application/parent-001/data/{data_key}/{data_file_id}.txt'
    ...
    def test_store_data__json(self):                                                            # Test JSON data storage
        with self.service__store_data as _:
            ...
            expected_data_file = f'{self.test_namespace}/data/semantic-file/logs/application/parent-001/data/{data_key}/{data_file_id}.json'
    ...
    def test_store_data__binary(self):                                                          # Test binary data storage
        with self.service__store_data as _:
            ...
            result = _.store_data(cache_id     = self.parent_cache_id                           ,
                                 data         = self.test_binary                                ,
                                 data_type    = Enum__Cache__Data_Type.BINARY                   ,
                                 data_key     = data_key                                        ,
                                 data_file_id = data_file_id                                    ,
                                 namespace    = self.test_namespace                             )

            expected_data_file = f'{self.test_namespace}/data/semantic-file/logs/application/parent-001/data/{data_key}/{data_file_id}.bin'
    ...
    def test_store_data__without_data_key(self):                                                # Test without data_key (direct in data folder)
        with self.service__store_data as _:
            data_file_id = Safe_Str__Id('direct-file')
            ...
            assert type(result) is Schema__Cache__Store__Data__Response
            created_file = result.data_files_created[0]
            assert f'/data/{data_file_id}.txt' in created_file
    ...
    def test_store_data__multiple_to_same_parent(self):                                         # Test multiple data files for same parent
        ...
        with self.service__store_data as _:
            ...
            for i in range(3):
                data_file_id = Safe_Str__Id(f'multi-data-{i:03d}')
                result = _.store_data(cache_id     = self.parent_cache_id                       ,
                                     data         = f"data file {i}"                            ,
                                     data_type    = Enum__Cache__Data_Type.STRING               ,
    ...
    def test_store_data__path_traversal_safety(self):                                           # Test that url_join_safe prevents path traversal
        with self.service__store_data as _:
            ...
            data_key     = Safe_Str__File__Path('../../../etc')                                 # Dangerous path
            data_file_id = Safe_Str__Id('passwd')

            result = _.store_data(cache_id     = self.parent_cache_id                           ,
                                 data         = "path traversal attempt"                        ,
                                 data_type    = Enum__Cache__Data_Type.STRING                   ,
                                 data_key     = data_key                                        ,
                                 data_file_id = data_file_id                                    ,
                                 namespace    = self.test_namespace                             )
