...
class Service__Cache__Retrieve(Type_Safe):                                            # Service layer for cache retrieval operations
    ...
    def retrieve_by_id(self, cache_id  : Random_Guid,
        ...
        id_ref = self.cache_service.get_id_reference(cache_id, namespace)               # Check if ID exists first
        
        if id_ref is None:
            return None                                                                 # Not found

        if self._is_expired(id_ref):                                                    # Check if expired (if TTL handling is implemented)
            return None                                                                 # Expired = Gone
    ...
    @type_safe
    def get_expired_error(self, cache_id   : Random_Guid,
                                expired_at : Safe_UInt  ,
                                ttl_hours  : Safe_UInt  ,
